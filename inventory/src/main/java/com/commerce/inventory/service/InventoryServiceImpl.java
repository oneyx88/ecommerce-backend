...
import com.commerce.inventory.client.ProductClientService;
...
public class InventoryServiceImpl implements InventoryService {
    ...
    // ----------------------------------------------------------
    @Override
    public List<InventoryResponse> getSellerInventories(String sellerId) {

        // 1️⃣ 调用 product-service 查询 seller 所有商品
        List<ProductResponse> products = productClientService.getSellerProducts(sellerId);
        ...
        return inventories.stream()
                .map(inv -> {
                    InventoryResponse dto = modelMapper.map(inv, InventoryResponse.class);
                    dto.setLastUpdated(inv.getUpdatedAt().toString());
                    return dto;
                })
                .toList();
    }
    ...
    // ----------------------------------------------------------
    @Override
    public InventoryResponse createSellerInventory(String sellerId, Long productId, InventoryRequest request) {

        validateSellerProduct(sellerId, productId);
...
//        publishEvent(productId, "CREATE");
    ...
    // ----------------------------------------------------------
    // 校验该 product 是否属于 seller（重要）
    // ----------------------------------------------------------
    private void validateSellerProduct(String sellerId, Long productId) {
        List<ProductResponse> products = productClientService.getSellerProducts(sellerId);

        boolean owns = products.stream().anyMatch(p -> p.getProductId().equals(productId));

        if (!owns) {
            throw new ApiException("Product does not belong to seller", HttpStatus.FORBIDDEN);
        }
    }
