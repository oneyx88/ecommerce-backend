...
class InventoryController {
    ...
    @GetMapping("/seller")
    @PreAuthorize("hasRole('SELLER')")
    public ResponseEntity<List<InventoryResponse>> getSellerInventories(
            @RequestHeader("X-User-Id") String sellerId
    ) {
        List<InventoryResponse> response = inventoryService.getSellerInventories(sellerId);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/seller/{productId}")
    @PreAuthorize("hasRole('SELLER')")
    public ResponseEntity<InventoryResponse> createSellerInventory(
            @PathVariable Long productId,
            @RequestHeader("X-User-Id") String sellerId,
            @RequestBody InventoryRequest request
    ) {
        InventoryResponse response = inventoryService.createSellerInventory(sellerId, productId, request);
        return ResponseEntity.ok(response);
    }
